...
class IP(Layer2):
    ...
    def dataAvailable(self):
        ...
        if (_targetIP != '1.4.1.2'):
            _echoPacket = dpkt.icmp.ICMP.Unreach(
            )
            _icmpPacket = dpkt.icmp.ICMP(
            ...
            _ipPacket = dpkt.ip.IP(
                src = utils.ipStringToBytes(_targetIP),
                dst = utils.ipStringToBytes(_sourceIP),
                p = dpkt.ip.IP_PROTO_ICMP,
                data = _icmpPacket.pack(),
                id = _packet.rawPacket.data.id,
                ttl = 64,
            )
            _ethernetPacket = ethernet.RawPacket(
                _packet.ethernetDestination,
                _packet.ethernetSource,
                self.ethernetType,
                _ipPacket.pack(),
            )
            self.sendData(_ethernetPacket, _packet.ethernetSource)